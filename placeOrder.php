        ...
        if($_SESSION["user_type"] === "user") {
            ...
            if((mysqli_num_rows($userCart) != 0)) {
                ...
                while($userCartRow = $userCart->fetch_assoc()) {
                    ...
                    $cartQuantity = $userCartRow['quantity'];

                    // Check if order/s is above the stock
                    $sqlProd = "SELECT stock FROM storeContent WHERE id = '$product_id'";
                    $prod = mysqli_query($conn, $sqlProd);
                    $prodRow = $prod->fetch_assoc();
                    $currStock = $prodRow['stock'];

                    if ($currStock < $cartQuantity){
                        $higherThanStock = $higherThanStock + 1;
                    }
                }
                
                if ($higherThanStock == 0){
                    $sqlUserCart = "SELECT * FROM shopping_cart WHERE user_id = $user_id";
                    $userCart = mysqli_query($conn, $sqlUserCart);
                    while($userCartRow = $userCart->fetch_assoc()) {
                    ...
                    echo "
                    <script>
                        alert('Check the stock and quantity.');
